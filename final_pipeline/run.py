...
def deduplicate_quadrilaterals_by_colors(quads, color_template_freq, dedup_thresh):
	"""
	...
	When merging, the quadrilateral vertices are averaged.
	Since a template can be found on the border of two masks only, if more than two are found,
	we keep only the two colors that are most frequent among templates, as an heuristic.
	Args:
	...
	Returns:
		List of dicts {'vertices': quad_pts, 'colors': [color_indices]} with merged colors and averaged vertices
	"""
	deduped = []
	for i, quad in enumerate(quads):
		...
		for j, existing in enumerate(deduped):
			...
			if dist < dedup_thresh:
				# Merge colors
				merged_colors = set(existing['colors']).union(quad_colors)
				# Heuristic: keep only the two most frequent colors among templates
				# color_template_freq is an array/list of color indices, most frequent first
				# Sort merged_colors by their order in color_template_freq
				sorted_colors = sorted(
					merged_colors,
					key=lambda c: color_template_freq.index(c) if c in color_template_freq else float('inf')
				)
				# Keep at most two
				existing['colors'] = sorted_colors[:2]
				# Average vertices
				v1 = np.array(existing['vertices'], dtype=np.float32)
				v2 = np.array(quad_pts, dtype=np.float32)
				avg_vertices = (v1 + v2) / 2.0
				existing['vertices'] = avg_vertices
				found_duplicate = True
				break
		if not found_duplicate:
			# Also apply the color frequency heuristic here in case quad_colors > 2
			sorted_colors = sorted(
				quad_colors,
				key=lambda c: color_template_freq.index(c) if c in color_template_freq else float('inf')
			)
			deduped.append({'vertices': np.array(quad_pts, dtype=np.float32), 'colors': sorted_colors[:2]})
	return deduped

def symbol_detection_pipeline(image_path, templates, reference_colors, rgb_threshold, black_val_threshold, white_sat_threshold, white_val_threshold, ncc_threshold, min_line_length_percent, adjacency_radius, dedup_thresh, scale, num_octaves, angle_tolerance, distance_tolerance, color_template_freq):
	"""
	...
	all_quads = deduplicate_quadrilaterals_by_colors(all_quads, color_template_freq, dedup_thresh)
